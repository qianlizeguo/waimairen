...
//	 if($checkps['canps'] != 1) $this->message('shop_notinps');  
  ...
  function savepxmyF(){
	...
	{  
	 ...
	 if($ruzhutype == 1){
		 if(empty($qiyeimgurl))  $this->message('请上传企业执照');
	 }
	 if(empty($zmimgurl))  $this->message('请上传正面的身份证图片');
	 if(empty($fanimgurl))  $this->message('请上传反面的身份证图片');
	 if(empty($foodimgurl))  $this->message('请上传食品流通证图片');
