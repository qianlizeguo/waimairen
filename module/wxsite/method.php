...
{  
	...
	#	print_r($shopshowtype);
	 ...
	 $shopcounts = $this->mysql->select_one( "select count(id) as shuliang  from ".Mysite::$app->config['tablepre']."order	 where suretime >= ".$firstday." and suretime <= ".$lastday."  and status = 3 and  shopid = ".$values['id']."" );
								...
								if($zongpointcount != 0 ){
									$shopstart = intval( round($zongpoint/$zongpointcount/2) );
								}else{
									$shopstart= 0;
								}
...
/* 	 print_r($backdata);
	...
	function gift(){
		...
		if($this->member['uid'] == 0)  $this->message('未登陆',$link); 
	  	$link = IUrl::creatUrl('wxsite/index'); 
		$giftlog = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."memberlog where type = 1 ");  //积分增减记录
 ...
 /* 发布跑腿 start   */
	function fabupaotui(){
		...
		$pttype = trim(IReq::get('pttype'));		// 	1为帮我送 2为帮我取
